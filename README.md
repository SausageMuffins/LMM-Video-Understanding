...
1.  **Data Ingestion & Initial Metadata:** Download the benchmark dataset (`challenge_data.csv`) from Hugging Face. Use `pytubefix` via the `download_vids_and_metadata.py` script to retrieve basic metadata (title, description, duration, captions) for each video URL and save it to `video_full_metadata.csv`.
...
*   **Assets & Datasets:**
    *   `lmms-lab/AISG_Challenge` (Hugging Face Dataset): Contains the benchmark video URLs and questions. The script `download_vids_and_metadata.py` saves the relevant part as `challenge_data.csv`.
...
*   **Command:**
    ```bash
    python download_vids_and_metadata.py
    ```
*   **Action:**
