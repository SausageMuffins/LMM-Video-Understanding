...
def slow_down_video(input_path, output_path, speed_factor=0.5):
    ...
    cmd = [
        ...
        '-crf', '23',       # Constant Rate Factor (quality, lower is better, 18-28 typical) - Added reasonable default
        '-c:a', 'aac',      # Audio codec
        '-b:a', '128k',     # Audio bitrate - Added reasonable default
        output_path,
    ...
    try:
        subprocess.run(cmd, check=True, capture_output=True, text=True) # Capture output for potential debugging
    ...
    def time_to_seconds(time_str):
        ...
        try:
            ...
            if len(time_parts) == 3:
                ...
                print(f"Warning: Unexpected time format '{time_str}'. Assuming HH:MM:SS,ms.")
...
def main():
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing Videos"):
        ...
        if not isinstance(video_path, str) or not video_path:
            print(f"Warning: Invalid video path at index {idx}: {video_path}. Skipping.")
        ...
        try:
            ...
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
        except ValueError: # Handles case where video_path is not within input_video_base_dir
             print(f"Warning: Video path '{video_path}' does not seem to be relative to '{input_video_base_dir}'. Constructing output path differently.")
        ...
        try:
            ...
            if pd.notna(row['caption']) and isinstance(row['caption'], str):
                ...
                adjusted_caption = adjust_srt_timestamps(row['caption'], speed_factor=default_speed_factor)
                df.at[idx, 'slowed_caption'] = adjusted_caption
            else:
                print(f"No valid caption found for {video_path} or caption is not text.")
                df.at[idx, 'slowed_caption'] = row['caption'] # Keep original if not string or NaN
    ...
    if 'duration' in output_df.columns:
        ...
        print("Warning: 'duration' column not found in metadata. Skipping duration adjustment.")


    # Remove temporary and status columns before saving
    columns_to_drop = ['slowed_video_path', 'slowed_caption', 'processing_status', 'processing_error']
    output_df = output_df.drop(columns=[col for col in columns_to_drop if col in output_df.columns], axis=1)

    # --- Save Updated Metadata ---
    print(f"Saving updated metadata to {output_metadata_file}...")
    try:
        output_df.to_csv(output_metadata_file, index=False, encoding='utf-8-sig') # Use utf-8-sig for better Excel compatibility
    except Exception as e:
