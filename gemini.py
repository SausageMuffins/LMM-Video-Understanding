...
def main():
    ...
    parser.add_argument(
        "--output_csv_path",
        default="challenge_data_gemini_debug.csv",
    ...
    for idx, row in df_challenge_data.iterrows():
        ...
        if args.v2:
            ...
            if len(parts) > 1:
                thinking_steps = parts[1].split("FINAL ANSWER:")[0].strip()
                final_answer = parts[1].split("FINAL ANSWER:")[1].strip()
                answer = final_answer
            else:
                print("Could not extract thinking steps and final answer.")
        else:
            final_answer = answer
