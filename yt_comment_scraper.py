...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Scrape YouTube comments from videos listed in a CSV file."
    )
    parser.add_argument(
        "--input",
        "-i",
    ...
    try:
        ...
        while response and items_processed < max_results:
            for item in response.get("items", []):
                ...
                if (
                    "snippet" in item
                    and "topLevelComment" in item["snippet"]
                    and "snippet" in item["snippet"]["topLevelComment"]
                ):
...
def sanitize_filename(title):
    ...
    if not isinstance(title, str):
        ...
        print(
            f"  Warning: sanitize_filename received non-string input: {type(title)}. Using default."
        )
...
def main():
    ...
    try:
        ...
        if missing_cols:
            raise ValueError(
                f"Input CSV missing required columns: {', '.join(missing_cols)}"
            )
        ...
        for index, row in df.iterrows():
            try:  # Start of per-row error handling
                ...
                if not video_id:
                    print(
                        f"  Could not extract valid video ID from URL: {youtube_url}. Skipping."
                    )
                ...
                if comments:
                    ...
                    try:
                        ...
                        print(
                            f"  Error saving comments CSV for video ID {video_id}: {e_csv}"
                        )
                elif (
                    verbose or not "disabled comments" in str(e).lower()
                ):  # Only print 'no comments' if not known to be disabled
                    print(
                        f"  No comments found or retrieved for video: {display_title} (ID: {video_id})"
                    )
